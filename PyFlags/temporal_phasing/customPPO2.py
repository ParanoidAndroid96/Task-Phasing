...
class CustomPPO2(PPO2):
        ...
        def learn(self, total_timesteps, iteration, callback=None, log_interval=1, tb_log_name="PPO2",
              reset_num_timesteps=True):
                ...
                with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
                    ...
                    update = 0
                    while(self.runner.policy_prob <= 300.1001):
                    #for update in range(1, n_updates + 1):
                        ...
                        # Unpack
                        obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward, likelihood_ratio, AI_used = rollout

                        callback.on_rollout_end()
		     
                        returns = likelihood_ratio * returns
...
class Runner(AbstractEnvRunner):
    ...
    def policy_decide(self, policy_prob):
        return np.random.rand() > self.policy_prob
    ...
    def _run(self):
        ...
        mb_obs, mb_rewards, mb_actions, mb_values, mb_dones, mb_neglogpacs, mb_likelihood_ratio = [], [], [], [], [], [], []
        ...
        for step in range(self.n_steps):
            ...
            gen_eps = self.env.get_attr("gen_eps")[0]
            time_steps = self.env.get_attr("time_steps")[0]
            alive = self.env.get_attr("alive")[0]
            ep_len = self.env.get_attr("ep_len")[0]

            #Choose between current policy or other policy and find likelihood update
            if(self.policy_decide(self.policy_prob)):#if(time_steps > self.thresh_steps):# and alive != 0):
                rand_prob = 0.2 
                control_actions = self.env.env_method("control", self.obs)[0]
            ...
            if(self.dones):
                self.likelihood_ratio = 1.0
                print(self.policy_prob, self.thresh_steps, self.norm_w)
                if(self.ep_reward != []):
            ...
            if(episode % 50 == 0 and episode != self.last_trust_update):
                ...
                if(self.last_trust_update < 0 or (self.cur_mean_reward >= self.prev_mean_reward)):
                    self.policy_prob += min(self.policy_prob+self.get_phase_step(), 1.0)
                    self.thresh_steps += int(ep_len / 10)
                else:
                    #self.policy_prob = max(self.policy_prob-get_phase_step(), 0.1)
                    self.thresh_steps = max(self.thresh_steps-int(ep_len / 10), 0.0)
            ...
            if(control_action_prob != 1.0):
                    ...
                    self.likelihood_ratio = 1.0#(action_probs) / control_action_prob
                    if(abs(control_action_prob - rand_prob) < 0.0001):
                        action_probs = np.array([0.5, 0.5, 0.5]) * control_action_prob
                    else:
                        action_probs = np.array([1.0, 1.0, 1.0]) * control_action_prob
            ...
            mb_likelihood_ratio.append(self.likelihood_ratio)
            mb_neglogpacs.append(neglogpacs)

            #Before sending the action see if the steer is close to the programmed AI's steer
            #If it is not then tell the agent to take an action to steer using programmed AI
            #Else tell it to steer using the RL model

            #Execute action in the environment to find the reward
            clipped_actions = control_actions
            # Clip the actions to avoid out of bound error
            if isinstance(self.env.action_space, gym.spaces.Box):
                clipped_actions = np.clip(control_actions, [-1, -1, -1], [1, 1, 1]) #self.env.action_space.low, self.env.action_space.high)
        ...
        mb_likelihood_ratio = np.asarray(mb_likelihood_ratio, dtype=np.float32)
        ...
        mb_likelihood_ratio = mb_likelihood_ratio.reshape((1, mb_likelihood_ratio.shape[0]))

        
        print("Proportions RL_used = "+str(RL_used)+" AI_used = "+str(self.n_steps-RL_used))

        mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, true_reward, mb_likelihood_ratio = \
            map(swap_and_flatten, (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, true_reward, mb_likelihood_ratio))

        return mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states, ep_infos, true_reward, mb_likelihood_ratio, AI_used
